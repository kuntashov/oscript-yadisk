...
Процедура ТестДолжен_ПроверитьПолучениеСвойствФайлаВКорзине() Экспорт
    ...
    ЯндексДиск.Удалить(ПутьНаДиске, Ложь, Истина);

    СвойстваФайла = ЯндексДиск.ПолучитьСвойстваРесурсаВКорзине(ИмяФайла);

    юТест.ПроверитьИстину(СвойстваФайла.Свойство("name"));
    юТест.ПроверитьРавенство(ИмяФайла, СвойстваФайла.name);

    // Прибираем за собой.
    ЯндексДиск.ОчиститьКорзину();
...
Функция ПолучитьТокенАвторизации()    
    ...
    Если ПустаяСтрока(Токен) Тогда    
        ...
        Если (Новый Файл(ПутьКФайлуСТокеном)).Существует() Тогда
            ...
            ЧтениеТекста.Закрыть();
            Лог.Отладка("Токен получен из файла " + ПутьКФайлуСТокеном);
        КонецЕсли;    
    Иначе
        Лог.Отладка("Токен получен из переменной среды YADISK_OAUTH_TOKEN");
    КонецЕсли;    
    Если ПустаяСтрока(Токен) Тогда
        Лог.Отладка("Токен пустой");
    КонецЕсли;    
    Возврат Токен;
КонецФункции
